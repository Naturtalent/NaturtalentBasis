...
/**
 * Generiert die Regex-Pattern fuer die Standardtextsuche.
 * 
 * (kopiert von org.eclipse.search.internal.core.text.PatternConstructor)
 * 
 * @author dieter
 *
 */
public class PatternConstructor
{
			...
			boolean isWholeWord) throws PatternSyntaxException
	{
		if (isRegex)
		{
			...
			Assert.isTrue(!isWholeWord,
					"isWholeWord unsupported together with isRegex"); //$NON-NLS-1$
		}
		else
		{
	...
	/**
	 * Copied from {@link org.eclipse.jface.text.FindReplaceDocumentAdapter}' to
	 * support '\R'
	 *
	 * @param findString
					...
					case '\\':
						if (i + 1 < length)
						{
							...
							else if (ch1 == 'R')
							{
								...
								{
									String msg = Messages.PatternConstructor_error_line_delim_position;
	...
	public static StringBuilder appendAsRegEx(boolean isStringMatcher,
			String pattern, StringBuilder buffer)
	{
	 ...
	 *            the replace pattern
	 * @param foundText
	 *            the found pattern to be replaced
	 * @param lineDelim
	 *            the line delimiter to use for \R
	 * @return a replace pattern with escaped characters substituted by the
	 *         respective characters
	 * @since 3.4
	 */
	public static String interpretReplaceEscapes(String replaceText,
			String foundText, String lineDelim)
	{
		return new ReplaceStringConstructor(lineDelim)
				.interpretReplaceEscapes(replaceText, foundText);
	}

	/**
	 * Copied from {@link FindReplaceDocumentAdapter}}
	 *
	 * FindReplaceDocumentAdapter with contributions from: Cagatay Calli
	 * <ccalli@gmail.com> - [find/replace] retain caps when replacing -
	 * https://bugs.eclipse.org/bugs/show_bug.cgi?id=28949 Cagatay Calli
	 * <ccalli@gmail.com> - [find/replace] define & fix behavior of retain caps
	 * with other escapes and text before \C -
	...
	private static class ReplaceStringConstructor
	{
				...
				String foundText)
		{
			...
			/*
			 * every string we did not check looks mixed at first so initialize
			 * retain case mode with RC_MIXED
			 */
			...
			for (int i = 0; i < length; i++)
			{
				...
				else if (ch == '$')
				{
					 ...
					 * $00, $000, etc. are interpreted as $0 and $01, $001, etc.
					 * are interpreted as $1, etc. . If we support \0 as
					 * replacement pattern for capturing group 0, it would not
					 * be possible any more to write a replacement pattern that
					 * appends 0 to a capturing group (like $0\0). The fix is to
					 * interpret \00 and $00 as $0\0, and \01 and $01 as $0\1,
					 * etc.
					 */
					if (i + 2 < length)
					{
			...
			if (inEscape)
			{
				// '\' as last character is invalid, but we still add it to get
				// an error message
				buf.append('\\');
			}
			return buf.toString();
		...
		/**
		 * Interprets the escaped character <code>ch</code> at offset
		 * <code>i</code> of the <code>replaceText</code> and appends the
		 * interpretation to <code>buf</code>.
		 *
		 * @param ch
		 ...
		 *            the output buffer
		 * @param replaceText
		 *            the original replace pattern
		 * @param foundText
		 *            the found pattern to be replaced
		 * @return the new offset
		 * @since 3.4
		 */
		private int interpretReplaceEscape(final char ch, int i,
				StringBuilder buf, String replaceText, String foundText)
		{
			int length = replaceText.length();
			switch (ch)
				{
					...
					case 'c':
						...
						{
							String msg = Messages.PatternConstructor_error_escape_sequence;
							throw new PatternSyntaxException(msg, replaceText,
									i);
						}
						break;
					...
					case 'x':
						if (i + 2 < length)
						{
							...
							{
								String msg = Messages.PatternConstructor_error_hex_escape_sequence;
								throw new PatternSyntaxException(msg,
										replaceText, i);
							}
					...
					case 'u':
						if (i + 4 < length)
						{
							...
							{
								String msg = Messages.PatternConstructor_error_unicode_escape_sequence;
								throw new PatternSyntaxException(msg,
										replaceText, i);
							}
							interpretRetainCase(buf, (char) parsedInt);
					...
					case 'C':
						if (foundText.toUpperCase().equals(foundText)) // is
																		// whole
																		// match
																		// upper-case?
							fRetainCaseMode = RC_UPPER;
						else if (foundText.toLowerCase().equals(foundText)) // is
																			// whole
																			// match
																			// lower-case?
							fRetainCaseMode = RC_LOWER;
						else if (Character.isUpperCase(foundText.charAt(0))) // is
																				// first
																				// character
																				// upper-case?
		...
		/**
		 * Interprets current Retain Case mode (all upper-case,all
		 * lower-case,capitalized or mixed) and appends the character
		 * <code>ch</code> to <code>buf</code> after processing.
		 *
		 * @param buf
		 *            the output buffer
		 * @param ch
