 ...
 * @author dieter
 *
 * Copies all snippet perspectives to perspective stack called "MainPerspectiveStack" 
 * In order to register/reset perspective and not have to sync two copies in e4xmi.
 */
public class CopyPerspectiveSnippetProcessor
{
	...
	public void execute(EModelService modelService, MApplication application)
	{
		MPerspectiveStack perspectiveStack = (MPerspectiveStack) modelService
				.find(MAIN_PERSPECTIVE_STACK_ID, application);
		...
		for (MUIElement snippet : application.getSnippets())
		{
			...
			{
				MPerspective perspectiveClone = (MPerspective) modelService
						.cloneSnippet(application, snippet.getElementId(), null);
