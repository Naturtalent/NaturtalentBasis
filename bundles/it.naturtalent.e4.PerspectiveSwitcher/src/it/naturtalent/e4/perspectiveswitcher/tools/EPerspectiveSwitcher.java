...
public class EPerspectiveSwitcher
{
	...
	private EventHandler labelHandler = new EventHandler()
	{
		...
		public void handleEvent(Event event)
		{
			...
			MWindow perspectiveWindow = perspective.getContext().get(
					MWindow.class);
	...
	private EventHandler selectionHandler = new EventHandler()
	{
		...
		public void handleEvent(Event event)
		{
			...
			MWindow stackWindow = modelService.getContainingContext(
					perspectiveStack).get(MWindow.class);
			if (window != stackWindow)
				return;

			MPerspective selectedElement = perspectiveStack
					.getSelectedElement();
	...
	private EventHandler childrenHandler = new EventHandler()
	{
		...
		public void handleEvent(Event event)
		{
			...
			{
				System.out.println("EPerspectiveSwitcher:childrenHandler ADD");
						...
						UIEvents.EventTags.NEW_VALUE))
				{
					...
					// Adding invisibleelements is a NO-OP
			...
			{
				System.out.println("EPerspectiveSwitcher:childrenHandler REMOVE");
						...
						UIEvents.EventTags.OLD_VALUE))
				{
					...
					// Removing invisibleelements is a NO-OP
	...
	/**
	 * Sets the UI control associated with the perspective switcher. The
	 * perspective switcher subscribes to topics only once a non-null ui control
	 * is passed and unsubscribes from all topics if a null value is received.
	 * 
	 * @param uiControl
