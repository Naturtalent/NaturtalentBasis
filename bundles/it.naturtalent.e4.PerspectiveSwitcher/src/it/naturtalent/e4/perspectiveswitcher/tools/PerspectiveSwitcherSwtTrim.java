 ...
 ******************************************************************************/
package it.naturtalent.e4.perspectiveswitcher.tools;

import it.naturtalent.e4.perspectiveswitcher.E4WorkbenchMessages;
...
public class PerspectiveSwitcherSwtTrim implements IPerspectiveSwitcherControl
{
	...
	void createControl(Composite parent, MToolControl control)
	{
		...
		// Listet alle momentan definierten Perspektiven
	...
	void paintContainer(PaintEvent event)
	{
		...
		int h = size.y;
		int[] simpleCurve = new int[]
			{ 0, h - 1, 1, h - 1, 2, h - 2, 2, 1, 3, 0 };
	...
	/* Shortcut und Icon hinzufuegen
	 * @see it.naturtalent.e4.perspectiveswitcher.tools.IPerspectiveSwitcherControl#addPerspectiveShortcut(org.eclipse.e4.ui.model.application.ui.advanced.MPerspective)
	 */
	@Override
	public void addPerspectiveShortcut(MPerspective perspective)
	{
		...
		String test = perspective.getIconURI();
	...
	// re-inject bei Aenderung Praeferenz 'E4PerspectiveSwitcherPreferences.SHOW_TEXT' 
	@Inject
	void setShowText(
			@Preference(value = E4PerspectiveSwitcherPreferences.SHOW_TEXT, nodePath = E4PerspectiveSwitcherPreferences.ROOT_PREFERENCES_NODE)
			boolean showShortcutText)
	{
		...
		for (ToolItem item : toolBar.getItems())
		{
			...
			{
				// Textanzeige abschalten - Originaltext in Tooltip uebernommen
				item.setText(_null);
	...
	public ToolItem getToolItemFor(MPerspective perspective)
	{
		if (toolBar == null || toolBar.isDisposed())
			return null;
	...
	public MPerspective getFirstShortcut()
	{
		if (toolBar == null || toolBar.isDisposed())
			return null;
	...
	public MPerspective getNextShortcut(MPerspective activePerspective)
	{
		if (toolBar == null || toolBar.isDisposed())
			return null;
		...
		for (int i = 2; i < toolBar.getItems().length && toolItem == null; i++)
		{
			...
			if (!item.isDisposed())
			{
				Object obj = item.getData();
				if (obj instanceof MPerspective)
					if(!obj.equals(activePerspective))					
						return (MPerspective) obj;
			}
		}
