...
import it.naturtalent.e4.project.expimp.dialogs.RestoreNtProjektDialog;
...
public class RestoreAction extends Action
{
	...
	public void run()
	{
		...
		RestoreNtProjektDialog dialog = new RestoreNtProjektDialog(shell);
		if(dialog.open() == ProjectImportDialog.OK)
		{
			//doRun(dialog); 
			
			String importFileName = dialog.getImportFile();
			File restoreDir = createRestoreDir();
			unzipArchiv(importFileName, restoreDir);
	...
	// das Haupt-Restoreverzeichnis erzeugen
	private File createRestoreDir()
	{
		// ein temporaeres Verzeichnis
		File tempDir = FileUtils.getTempDirectory();
		
		// BackUp-Verzeichnis in das die Projekte exportiert werden
		File restoreDir = new File(FileUtils.getTempDirectoryPath(), "restore");
		if(restoreDir.exists())
		{
			try
			{
				// Restore-Verzeichnis vorher loeschen
		...
		// Restore-Verzeichnis neu erzeugen
		return (restoreDir.mkdir()) ? restoreDir : null; 
	}
