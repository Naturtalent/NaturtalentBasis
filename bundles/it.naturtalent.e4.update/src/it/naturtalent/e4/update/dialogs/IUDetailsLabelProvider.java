...
public class IUDetailsLabelProvider extends ColumnLabelProvider implements
		ITableLabelProvider, IFontProvider
{
	...
	public IUDetailsLabelProvider(FilteredTree filteredTree,
			IUColumnConfig[] columnConfig, Shell shell)
	{
	...
	public String getColumnText(Object element, int columnIndex)
	{
		...
		IInstallableUnit iu = ProvUI
				.getAdapter(element, IInstallableUnit.class);
		if (iu == null)
		{
			if (columnIndex == 0)
			{
		...
		switch (columnContent)
			{
				...
				case IUColumnConfig.COLUMN_NAME:
					...
					if (name != null)
						return name;
					// If the iu name is not available, we return blank if we
					// know know we are
					// showing id in another column. Otherwise we return id so
					// the user doesn't
					// see blank iu's.
					if (showingId)
						return BLANK;
				...
				case IUColumnConfig.COLUMN_SIZE:
					if (element instanceof IIUElement
							&& ((IIUElement) element).shouldShowSize())
						...
						public void run()
						{
							if (shell != null && !shell.isDisposed())
								fireLabelProviderChanged(new LabelProviderChangedEvent(
										IUDetailsLabelProvider.this, element));
						}
					});
	...
	private String getFormattedSize(long size)
	{
		...
		{
			long kb = size / 1000L;
			return NLS.bind(ProvUIMessages.IUDetailsLabelProvider_KB,
					NumberFormat.getInstance().format(new Long(kb)));
		}
		return NLS.bind(ProvUIMessages.IUDetailsLabelProvider_Bytes,
				NumberFormat.getInstance().format(new Long(size)));
	}
	...
	public Font getFont(Object element)
	{
		...
		{
			return FilteredTree.getBoldFont(element, filteredTree,
