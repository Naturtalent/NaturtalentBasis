...
public class NavigatorDropAdapter extends PluginDropAdapter implements IOverwriteQuery {
    ...
    private IStatus ok() {
        return new Status(IStatus.OK, PlatformUI.PLUGIN_ID, 0,
                DataTransferMessages.DropAdapter_ok, null);
    }
    ...
    private void openError(IStatus status) {
        ...
        String genericTitle = DataTransferMessages.DropAdapter_title;
	...
	public boolean performDrop(final Object data)
	{
		...
		else
		{
			...
			{
				// import von ausserhalb Eclipse (kein delete der Quelle)
			...
			else
			{
				...
				if (msMail instanceof IMailTransfer)
				{
					IMailTransfer iMailTransfer = (IMailTransfer) msMail;
					IMailData iMailData = ((IMailTransfer) msMail)
							.nativeToJava(iMailTransfer, currentTransfer);
					if (iMailData != null)
					{
						final String[] fileData = ((IMailData)iMailData).getMailFiles();				
						if(fileData != null)
						{
							MultiStatus problems = new MultiStatus(PlatformUI.PLUGIN_ID, 0,
									DataTransferMessages.DropAdapter_problemImporting, null);
							...
							Display.getCurrent().asyncExec(new Runnable()
							{
								public void run()
								{
									...
									CopyFilesAndFoldersOperation operation = new CopyFilesAndFoldersOperation(
											getShell());
									operation.copyFiles(fileData, target);
								}
							});
							status = problems;
							result = status.isOK();
    ...
    private IStatus performResourceCopy(Shell shell, IResource[] sources) 
    {
        MultiStatus problems = new MultiStatus(PlatformUI.PLUGIN_ID, 1,
                DataTransferMessages.DropAdapter_problemsMoving, null);
	...
	private IStatus performResourceMove(IResource[] sources)
	{
		...
		else
		{
			ReadOnlyStateChecker checker = new ReadOnlyStateChecker(getShell(),
					DataTransferMessages.MoveResourceAction_title,
					DataTransferMessages.MoveResourceAction_checkMoveMessage);
    ...
    public String queryOverwrite(String pathString) {
        ...
        final String msg = NLS.bind(DataTransferMessages.DropAdapter_overwriteQuery, pathString);
        ...
        getDisplay().syncExec(new Runnable() {
            public void run() {
                MessageDialog dialog = new MessageDialog(
                        getShell(),
                        DataTransferMessages.DropAdapter_question, null, msg, MessageDialog.QUESTION, options, 0);
                dialog.open();
	...
	private IStatus validateTarget(Object target, TransferData transferType)
	{
		...
		{
			return info(DataTransferMessages.DropAdapter_targetMustBeResource);
		}
		IResource resource = (IResource) target;
		if (!resource.isAccessible())
		{
			return error(DataTransferMessages.DropAdapter_canNotDropIntoClosedProject);
		}
		IContainer destination = getActualTarget(resource);
		if (destination.getType() == IResource.ROOT)
		{
			return error(DataTransferMessages.DropAdapter_resourcesCanNotBeSiblings);
		}
		String message = null;
		// drag within Eclipse?
		if (LocalSelectionTransfer.getInstance().isSupportedType(transferType))
		{
			...
			{
				message = DataTransferMessages.DropAdapter_dropOperationErrorOther;
			}
			else
			{
		...
		else
		{
			...
			{							
				...
				{		
					IMailTransfer iMailTransfer = (IMailTransfer)msMail;
					IMailData iMailData = ((IMailTransfer)msMail).nativeToJava(iMailTransfer, transferType);
					if (iMailData != null)				
					{
						CopyFilesAndFoldersOperation copyOperation = new CopyFilesAndFoldersOperation(
								getShell());
						
						String [] filenames = iMailData.getFilenames();
						if (filenames != null)
						{
							message = copyOperation.validateImportDestination(
									destination, iMailData.getFilenames());
						}
					}
				}
			}
