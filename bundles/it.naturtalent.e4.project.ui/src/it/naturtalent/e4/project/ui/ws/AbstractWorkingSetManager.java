...
import it.naturtalent.e4.project.ui.utils.StatusUtil;
		...
		IWorkingSetManager, BundleListener, IExtensionChangeHandler
{
	...
	static int MAX_RECENT_WORKINGSET = 5;
	
	private Log log = LogFactory.getLog(AbstractWorkingSetManager.class);
	...
	*/
	...
	public void dispose()
	{
		...
		for (final Iterator iter = elementAdapters.values().iterator(); iter
				.hasNext();)
		{
			SafeRunner.run(new WorkingSetRunnable()
			{
	...
	{
		return (IWorkingSet) new WorkingSet(name, name, elements);
	}
	...
	{
		SortedSet visibleSubset = new TreeSet(
				WorkingSetComparator.getInstance());
		...
		return (IWorkingSet[]) visibleSubset
				.toArray(new IWorkingSet[visibleSubset.size()]);
	}
	...
	{
		return (IWorkingSet[]) recentWorkingSets
				.toArray(new IWorkingSet[recentWorkingSets.size()]);
	}
	 ...
	 * 
	 * @param workingSet
	 *            working set to added to the list of recently used working
	 *            sets.
	 */
	 ...
	 * 
	 * @param changeId
	 *            one of IWorkingSetManager#CHANGE_WORKING_SET_ADD
	...
	public void saveWorkingSetState(IMemento memento)
	{
		...
		// break the sets into aggregates and non aggregates. The aggregates
		// should be saved after the non-aggregates
		// so that on restoration all necessary aggregate components can be
		// found.
	...
	private void saveWorkingSetState(final IMemento memento, List list)
	{
		for (Iterator i = list.iterator(); i.hasNext();)
		{
			...
			SafeRunner.run(new WorkingSetRunnable()
			{
				...
				public void run() throws Exception
				{
					// create a dummy node to write too - the write could fail
					// so we
					// shouldn't soil the final memento until we're sure it
					// succeeds.
					XMLMemento dummy = XMLMemento
					...
					// if the dummy was created successfully copy it to the real
					// output
	...
	protected IWorkingSet restoreWorkingSet(final IMemento memento)
	{
		...
		if (factoryID == null)
		{
			...
			// IMemento.saveState, and should be restored using
			// WorkingSetFactory
		...
		/*
		final IElementFactory factory = WorkbenchPlugin.getDefault()
				.getElementFactory(factoryID);
				*/
		
		final IElementFactory factory = new WorkingSetFactory();

		if (factory == null)
		{
			log.error("Unable to restore working set - cannot instantiate factory: " + factoryID); //$NON-NLS-1$
			return null;
		}
		...
		if (adaptable[0] == null)
		{
			log.error("Unable to restore working set - cannot instantiate working set: " + factoryID); //$NON-NLS-1$
			return null;
		}
		if ((adaptable[0] instanceof IWorkingSet) == false)
		{
			log.error("Unable to restore working set - element is not an IWorkingSet: " + factoryID); //$NON-NLS-1$
			return null;
		}
	...
	protected void saveMruList(IMemento memento)
	{
		...
		while (iterator.hasNext())
		{
			...
			mruMemento.putString(IWorkbenchConstants.TAG_NAME,
					workingSet.getName());
		}
	}
	...
	*/
	...
	private IWorkingSetUpdater getUpdater(WorkingSetDescriptor descriptor)
	{
		...
		if (updater == null)
		{
			...
			{
				firePropertyChange(CHANGE_WORKING_SET_UPDATER_INSTALLED, null,
						updater);
				PlatformUI
						.getWorkbench()
						.getExtensionTracker()
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.IWorkingSetManager#createWorkingSetSelectionDialog(org
	 * .eclipse.swt.widgets.Shell, boolean, java.lang.String[])
	 */
	/*
	public IWorkingSetSelectionDialog createWorkingSetSelectionDialog(
	...
	*/
	...
	{
		// neues 'XMLMemento' erzeugen
		XMLMemento memento = XMLMemento
				.createWriteRoot(IWorkbenchConstants.TAG_WORKING_SET_MANAGER);
		
		// WorkingSets im 'XMLMemento' speichern
		saveWorkingSetState(memento);
		
		// MRUList im 'XMLMemento' speichern
		saveMruList(memento);

		// 'XMLMemento' im WorkingSet-File speichern
	...
	{
		// nothing - this is handled lazily. These items are only created as
		// needed by the getUpdater() and getElementAdapter() methods
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.IWorkingSetManager#addToWorkingSets(org.eclipse.core.runtime
	 * .IAdaptable, org.eclipse.ui.IWorkingSet[])
	 */
	public void addToWorkingSets(final IAdaptable element,
	...
	public void setRecentWorkingSetsLength(int length)
	{
		...
		if (length < 1 || length > 99)
			throw new IllegalArgumentException(
					"Invalid recent working sets length: " + length); //$NON-NLS-1$
		IPreferenceStore store = PrefUtil.getAPIPreferenceStore();
		store.setValue(
				IWorkbenchPreferenceConstants.RECENTLY_USED_WORKINGSETS_SIZE,
				length);
		// adjust length
		sizeRecentWorkingSets();
		*/
	...
	{
		/*
		IPreferenceStore store = org.eclipse.ui.internal.util.PrefUtil.getAPIPreferenceStore();
		return store
				.getInt(IWorkbenchPreferenceConstants.RECENTLY_USED_WORKINGSETS_SIZE);
				*/
		return MAX_RECENT_WORKINGSET;
	}
