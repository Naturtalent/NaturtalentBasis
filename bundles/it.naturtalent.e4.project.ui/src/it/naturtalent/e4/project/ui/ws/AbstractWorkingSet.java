...
public abstract class AbstractWorkingSet implements IAdaptable, IWorkingSet
{
	...
	public static final String TAG_AGGREGATE = "aggregate"; //$NON-NLS-1$
	 ...
	 * 
	 * @param name
	 *            the unique name for this working set
	 * @param label
	...
	public void connect(IWorkingSetManager manager)
	{
		Assert.isTrue(this.manager == null,
				"A working set can only be connected to one manager"); //$NON-NLS-1$
	...
	protected void fireWorkingSetChanged(String property, Object oldValue)
	{
		AbstractWorkingSetManager receiver = manager != null ? (AbstractWorkingSetManager) manager
				: Activator.getWorkingSetManager();
	...
	public void setLabel(String label)
	{
		...
		labelBoundToName = Util.equals(label, name); // rebind the label to the
														// name
