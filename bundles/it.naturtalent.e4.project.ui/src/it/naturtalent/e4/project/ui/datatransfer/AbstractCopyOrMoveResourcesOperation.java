...
abstract class AbstractCopyOrMoveResourcesOperation extends
	...
	protected IStatus computeMoveOrCopyStatus() {
		...
		if (resources == null) {
			markInvalid();
			return getErrorStatus("There is not enough information to complete the resource operation");
		}
		for (int i = 0; i < resources.length; i++) {
			...
			if (!resource.exists()) {
				markInvalid();
				return getErrorStatus("Resource no loger exist in Workspace");
			}

			// Are we really trying to move it to a different name?
			if (!isDestinationPathValid(resource, i)) {
				markInvalid();
				return getErrorStatus("Resource cannot be moved or copied to the same location and name");
			}
