...
public class WorkingSetResourceMapping extends ResourceMapping
{
			...
			IProgressMonitor monitor) throws CoreException
	{
		...
		try
		{
			...
			{
				ResourceMapping mapping = mappings[i];
				result.addAll(Arrays.asList(mapping.getTraversals(context,
						new SubProgressMonitor(monitor, 100))));
			}
			return (ResourceTraversal[]) result
					.toArray(new ResourceTraversal[result.size()]);
		} finally
		{
			monitor.done();
		}
	...
	private ResourceMapping[] getMappings()
	{
		...
		for (int i = 0; i < elements.length; i++)
		{
			...
			ResourceMapping mapping = WorkingSetAdapterFactory
					.getContributedResourceMapping(element);
			if (mapping == null)
			{
